...
# Four samples per second catches quarter-second fades and packet/state transitions
# that a beginning/middle/end screenshot check routinely misses.
...
for video in "$RAW"/*.mp4; do
  ...
  # 4x4 tiles at 480 px each -> 1920-wide sheets, 4 seconds per sheet at 4 fps.
  ffmpeg -nostdin -y -v warning \
    -i "$video" \
    -vf "fps=${FPS},scale=480:-2:flags=lanczos,tile=4x4:padding=4:margin=4" \
    -q:v 2 -fps_mode passthrough "$OUT/${scene}-%03d.jpg"
done
