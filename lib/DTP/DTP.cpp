...
DTP::DTP(uint8_t NAPInterval) {
  ...
  randomSeed(
      (uint64_t)((uint64_t)MathExtension.murmur64(MAC::getInstance()->random())
                     << 32 |
...
vector<DTPNAPTimeRecord> DTP::neighbors() { return this->activeNeighbors; }

DTPNAPTimeRecord DTP::getMyNAP() { return this->myNAP; }
...
bool DTP::checkIftransmissionIsColliding(uint32_t startTime, uint32_t endTime) {
  sort(this->activeNeighbors.begin(), this->activeNeighbors.end(),
       compareByStartTime);
  endTime %= this->NAPInterval * 1000;

  for (DTPNAPTimeRecord i : this->activeNeighbors) {
    if ((i.startTime < endTime && i.endTime > endTime) ||
...
bool DTP::checkIfCurrentPlanIsColliding() {
  return checkIftransmissionIsColliding(this->myNAP.startTime,
                                        this->myNAP.endTime);
}
...
void DTP::sendingDeamon() {
  ...
  if (request.timeLeft <= 0) {
    ...
    if (checkIftransmissionIsColliding(this->currentTime,
                                       (this->currentTime + duration)))
      return;
    Serial.println("Sending packet " + String(request.size) + " " +
                   String(request.target) + " " + String(request.ack) + " " +
                   String(request.timeout));

    DTPPacketUnkown *packet = (DTPPacketUnkown *)request.data;

    LCMM::getInstance()->sendPacketSingle(request.ack, request.target,
                                          request.data, request.size,
                                          DTP::receiveAck, request.timeout);

    this->sendingQueue.erase(this->sendingQueue.begin());
  } else {
    int timeLeftRes =
        (int)(request.timeLeft) - (currentTime - this->lastTickSendingDeamon);
...
void DTP::cleaningDeamon() {
  ...
  // This code removes all neighbors who have not been heard from in the last
  // two intervals. It also removes any routes that go through the removed
  // neighbors.
  ...
  while (neighbourIterator != this->activeNeighbors.end()) {
    ...
    printf("Going through %d last interval heard %d num of intervals elapsed "
           "%d \n",
           neighbour.id, neighbour.lastIntervalHeard, numOfIntervalsElapsed);

    if (neighbour.lastIntervalHeard - numOfIntervalsElapsed >
        DTP_NAP_TOLERANCE) {
      printf("ereasing %d %d \n", neighbour.lastIntervalHeard,
             numOfIntervalsElapsed);
      this->activeNeighbors.erase(neighbourIterator);
    } else {
      printf("going to save the neighbour\n");
      routingCacheTable[neighbour.id] =
          DTPRoutingItemFull{neighbour.id, 1, true};
      ...
      for (auto routing : neighbour.routes) {
        ...
        if (routingCacheTable.find(routing.first) == routingCacheTable.end()) {
          routingCacheTable[routing.first] =
              DTPRoutingItemFull{neighbour.id, route.distance};
        } else {
          bool sharable = true;
          if (this->numOfIntervalsElapsed &&
              this->previousActiveNeighbors.find(neighbour.id) !=
                  this->previousActiveNeighbors.end() &&
              this->previousActiveNeighbors[neighbour.id].routes.find(
                  routing.first) !=
                  this->previousActiveNeighbors[neighbour.id].routes.end() &&
              this->previousActiveNeighbors[neighbour.id]
                      .routes[routing.first]
                      .distance < route.distance) {
            printf("unsharable %d source %d", routing.first, neighbour.id);
            Serial.println("unsharable" + String(routing.first) + " source " +
                           String(neighbour.id));
            sharable = false;
          }
          if (route.distance < routingCacheTable[routing.first].distance &&
              sharable >= routingCacheTable[routing.first].sharable)
            routingCacheTable[routing.first] =
                DTPRoutingItemFull{neighbour.id, route.distance, sharable};
        }
      }
      neighbourIterator++;
    }
...
uint16_t DTP::sendPacket(uint8_t *data, uint8_t size, uint16_t target,
                         uint16_t timeout, DTP::PacketAckCallback callback) {
  uint16_t proxyId = this->getRoutingItem(target);
  if (!proxyId)
    return proxyId;

  DTPPacketGeneric *packet =
      (DTPPacketGeneric *)malloc(sizeof(DTPPacketGeneric) + size);
  ...
  this->addPacketToSendingQueue(true, proxyId, (uint8_t *)packet,
                                sizeof(DTPPacketGeneric) + size, timeout);
...
void DTP::addNeighbourIfNotExist(DTPNAPTimeRecord record) {
  ...
  for (unsigned int i = 0; i < this->activeNeighbors.size(); i++) {
    ...
    if (this->activeNeighbors[i].id == record.id) {
      ...
      for (auto route : record.routes) {
        if (this->activeNeighbors[i].routes.find(route.first) ==
            this->activeNeighbors[i].routes.end()) {
  ...
  if (!alreadyInNeighbours) {
    this->activeNeighbors.push_back(
        (DTPNAPTimeRecord){record.id, record.startTime, record.endTime,
                           this->numOfIntervalsElapsed, record.routes});
  }
}
...
void DTP::redistributePackets() {
  ...
  if (this->dataPacketWaiting &&
      this->dataPacketToParse->finalTarget != MAC::getInstance()->getId()) {
  ...
  if (this->ackPacketWaiting && this->ackPacketToParse->header.finalTarget !=
    ...
    if (proxyId != 0) {
      ...
      memcpy(dataToSend,
             ((LCMMPacketDataRecieve *)this->ackPacketToParse)->data,
             sizeToAllocate);
    } else {
  ...
  if (sizeToAllocate > 0 && dataToSend != nullptr) {
    ...
    this->addPacketToSendingQueue(true, proxyId, dataToSend, sizeToAllocate,
                                  3000);
  }
}
...
void DTP::parseDataPacket() {
  ...
  if (packet->finalTarget == MAC::getInstance()->getId()) {
    ...
    if (packet->type == DTP_PACKET_TYPE_DATA_SINGLE) {
      ...
      Serial.println("sending ack " + String(packet->id) + " to " +
                     String(packet->originalSender) + " throught " +
                     String(packet->lcmm.mac.sender) + " with id " +
                     String(responsePacket->header.id));

      this->addPacketToSendingQueue(false, packet->lcmm.mac.sender,
                                    (unsigned char *)responsePacket,
                                    sizeof(DTPPacketACK), 2000);
...
void DTP::parseNeigbours() {
  ...
  uint16_t numOfNeighbors =
      (size - sizeof(DTPPacketNAP)) / sizeof(NeighborRecord);
  ...
  if (!alreadyInNeighbours) {
    ...
    this->activeNeighbors.push_back((DTPNAPTimeRecord){
        senderId, startTime, endTime, this->numOfIntervalsElapsed, routes});
  }
...
size_t DTP::sizeOfRouting() {
  ...
  for (auto activeNeighbour : this->activeNeighbors) {
    for (auto route : activeNeighbour.routes) {
      if (find(alreadyAdded.begin(), alreadyAdded.end(), route.first) ==
          alreadyAdded.end()) {
...
uint32_t DTP::getTimeOnAirOfNAP() {
  ...
  uint16_t sizeOfRouting = sendableItems * sizeof(NeighborRecord) +
                           sizeof(DTPPacketNAP) + LCMM_OVERHEAD;
...
void DTP::NAPPlanRandom() {
  ...
  printf(("NAP planned for " + String(chosenTime) + " to " +
          String(chosenTime + timeOnAir))
             .c_str());
  this->NAPPlaned = true;
}

DTPNAPTimeRecordSimple DTP::getNearestTimeSlot(uint32_t ideal_min_time,
                                               uint32_t ideal_max_time,
  ...
  while (iterator != this->activeNeighbors.end()) {
    DTPNAPTimeRecordSimple neighbour = {(*iterator).startTime,
                                        (*iterator).endTime};
    ...
    if (nextNeighbour.startTime - neighbour.endTime >
        ideal_max_time - ideal_min_time) {
      possibleTimeSlots.push_back(
          DTPNAPTimeRecordSimple{neighbour.endTime, nextNeighbour.startTime});
    }
    iterator++;
  }
  ...
  int howMuchCanFit = (bestTimeSlot.endTime - bestTimeSlot.startTime) /
                      (ideal_max_time - ideal_min_time);
  Serial.println("how much can fit " + String(howMuchCanFit));
  if (howMuchCanFit > 1) {
    int margin = (bestTimeSlot.endTime - bestTimeSlot.startTime) -
                 howMuchCanFit * (ideal_max_time - ideal_min_time);
    ...
    bestTimeSlot.startTime =
        bestTimeSlot.startTime +
        timeSlotChosen * (sideMargin + ideal_max_time - ideal_min_time);
    bestTimeSlot.endTime =
        bestTimeSlot.startTime + (ideal_max_time - ideal_min_time);
    Serial.println("best time slot " + String(bestTimeSlot.startTime) + " to " +
                   String(bestTimeSlot.endTime));
  }
...
void DTP::NAPPlanInteligent() {
  ...
  for (DTPNAPTimeRecord i : this->activeNeighbors) {
    ...
    denominator += (i.endTime - i.startTime) *
                   (i.startTime + (i.startTime - i.endTime) / 2);
  ...
  if (this->currentTime > idealPoint - timeOnAir * 1.3 / 2) {
    Serial.println(String(this->currentTime) + " is over " +
                   String(timeOnAir * 1.3 / 2));
  ...
  DTPNAPTimeRecordSimple bestTimeSlot = getNearestTimeSlot(
      idealMinTime, idealMaxTime, 0, this->NAPInterval * 1000);

  if (bestTimeSlot.startTime == 0 && bestTimeSlot.endTime == 0) {
    this->NAPPlanRandom();
    return;
  }
  printf(("planning intelligent\n" + String(bestTimeSlot.startTime) + " to " +
          String(bestTimeSlot.endTime))
             .c_str());
...
void DTP::sendNAPPacket() {
  ...
  for (auto element : this->routingCacheTable) {
    if (element.second.sharable) {
      ...
      // Serial.println("sending " + String(element.first) + " " +
      // String(element.second.distance));
      sendableItems.push_back(
          NeighborRecord{element.first, element.second.distance});
    }
  }
  uint16_t sizeOfRouting =
      sendableItems.size() * sizeof(NeighborRecord) + sizeof(DTPPacketNAP);
  ...
  for (int i = 0; i < sendableItems.size(); i++) {
    printf("sending %d %d\n", packet->neighbors[i].id,
           packet->neighbors[i].distance);
    Serial.println("sending " + String(packet->neighbors[i].id) + " " +
                   String(packet->neighbors[i].distance));
  }

  this->addPacketToSendingQueue(false, 0, (unsigned char *)packet,
                                sizeOfRouting, 200);
...
void DTP::sendNAP() {
  ...
  // Serial.println("NAP planned" + String(NAPPlaned) + " nap sent " +
  // String(NAPSend) + "NAP current time: " + String(this->currentTime)+"
  // planning " + String(this->myNAP.startTime) + " to " +
  // String(this->myNAP.endTime));
  if (NAPPlaned && !NAPSend &&
      this->myNAP.startTime<this->currentTime &&this->myNAP.endTime> this
          ->currentTime) {
    sendNAPPacket();
...
void DTP::timeoutDeamon() {
  ...
  while (it != this->waitingForAck.end()) {
    ...
    if (currTime - this->lastTick >= waitingPacket.timeLeft) {
      ...
      // printf("time left %d \n", waitingPacket.timeLeft - (currTime -
      // this->lastTick));
...
void DTP::parseAckPacket() {
  if (ackPacketWaiting &&
      ackPacketToParse->header.finalTarget == MAC::getInstance()->getId()) {
    printf("\n\n parsing ack packet \n\n\n");
    ...
    while (it != this->waitingForAck.end()) {
      ...
      if (waitingPacket.id == id) {
        // Due to deletion in loop, iterator became
        // invalidated. So reset the iterator to next item.
        waitingPacket.callback(ackPacketToParse->header.type,
                               waitingPacket.timeout - waitingPacket.timeLeft);
        it = this->waitingForAck.erase(it);
      } else {
        it++;
      }
...
DTPStates DTP::getState() { return this->state; }
