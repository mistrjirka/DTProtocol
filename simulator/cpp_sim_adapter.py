...
from cpp_backend import CppNetwork, Tx


class CppSimNetwork(CppNetwork):
    """C++ protocol adapter with MCU-local clocks.

    The shared environment uses one global monotonic simulation time. Real
    microcontrollers do not: Arduino ``millis()`` restarts from zero after a
    reboot. Each C++ subprocess therefore receives ``global_time - boot_time``.

    Keeping this conversion in the adapter is important: topology, RF timing,
    failures and motion stay in the common EnvironmentKernel while MCU clock
    lifetime remains a property of the emulated firmware node.
    """
